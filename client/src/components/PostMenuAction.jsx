...
import { savedPost, deletePost, savePost } from "../apis/apiClient";
...
export const PostMenuAction = ({ post }) => {
  ...
  const saveMutation = useMutation({
    mutationFn: async (postId) => {
      const token = await getToken();
      return savePost(postId, token);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["savedPosts"] });
    },
    onError: (error) => {
      toast.error(error.response?.data || "Failed to update save status.");
    },
  });
